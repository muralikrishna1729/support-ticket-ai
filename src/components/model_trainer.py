...
class ModelTrainer:
    ...
    def initiate_model_trainer(self,
        ...
        try:
            ...
            logger.info(f"Best C: {grid.best_params_}")
            ...
            logger.info(f"Test F1 — Category: {f1_cat} | Issue Type: {f1_type}")

            # Reports
            logger.info("Category Report:\n" + classification_report(
                y_test_cat, best_cat_model.predict(X_test)
            ))
            logger.info("Issue Type Report:\n" + classification_report(
                y_test_type, issue_clf.predict(X_test)
            ))
